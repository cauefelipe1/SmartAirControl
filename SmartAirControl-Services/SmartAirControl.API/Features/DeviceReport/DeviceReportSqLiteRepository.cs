...
{
    public class DeviceReportSqLiteRepository : IDeviceReportRepository
    {
        private readonly DapperContext _dapperContext;

        public DeviceReportSqLiteRepository(DapperContext dapperContext) => _dapperContext = dapperContext;
        ...
        public IEnumerable<DeviceReportDTO> GetDeviceReportSensorByDateRange(DeviceReportSensorByDateRangeKey key)
        {
            const string SQL = BASE_DEVICE_REPORT_SELECT + @"
                WHERE 
                    ...
                    DATE(DeviceReadTS) >= @StartDate AND
                    DATE(DeviceReadTS) <= @EndDate";

            var param = new
            {
                key.DeviceId,
                StartDate = $"{key.StartDate:yyyy-MM-dd}",
                EndDate = $"{key.EndDate:yyyy-MM-dd}"
            };

            using (var conn = _dapperContext.CreateConnection())
            {
                var reports = conn.Query<DeviceReportDTO>(SQL, param);
